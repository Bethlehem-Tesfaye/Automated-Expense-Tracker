...
exports.parseReceiptWithGemini = parseReceiptWithGemini;
exports.validateReceiptWithGemini = validateReceiptWithGemini;
const generative_ai_1 = require("@google/generative-ai");
const genAI = new generative_ai_1.GoogleGenerativeAI(process.env.GEMINI_API_KEY);
/**
 * Cleans OCR text and removes empty lines
 */
function filterReceiptText(rawText) {
    const lines = rawText
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean);
 ...
 */
async function parseReceiptWithGemini(text, candidateCategories = []) {
    const filteredText = filterReceiptText(text);
    const categoriesPrompt = candidateCategories.length
        ? `You MUST choose the category from this exact list when it clearly fits: ${candidateCategories.join(", ")}. If none fits, return null for "category".`
...
async function validateReceiptWithGemini(text) {
    const filteredText = filterReceiptText(text);
    const prompt = `
...
`;
    try {
        ...
        if (typeof parsed?.isReceipt !== "boolean" ||

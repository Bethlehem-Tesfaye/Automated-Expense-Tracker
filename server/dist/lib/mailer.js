...
const oAuth2Client = new googleapis_1.google.auth.OAuth2(environments_1.env.GOOGLE_CLIENT_ID, environments_1.env.GOOGLE_CLIENT_SECRET);
oAuth2Client.setCredentials({
    refresh_token: environments_1.env.GOOGLE_REFRESH_TOKEN
});
...
async function sendMail({ to, subject, html, attachments = [] }) {
    try {
        const raw = buildRawMessage({
            from: environments_1.env.SENDER_EMAIL ?? process.env.SENDER_EMAIL ?? "no-reply@example.com",
            to,
            subject,
            html,
            attachments
        ...
        logger_1.logger.info({ to, subject, id: res.data?.id }, "Email sent");
        return res.data;
    }
    catch (err) {
        function extractErrorInfo(e) {
            ...
            if (e instanceof Error) {
                const response = typeof e === "object" && e !== null && "response" in e
                    ? e.response
                    : undefined;
